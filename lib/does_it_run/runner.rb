...
BLOCKED_PATTERNS = [
    ...
    def initialize(recipe, client: Solari::Client.new, cpu: 1, mem_mb: 2048, step_timeout_ms: 300_000, envs: {}, setup: [], verify: [])
      @recipe   = recipe
      @client   = client
      @cpu      = cpu
      @mem_mb   = mem_mb
...
def run
  ...
  Solari::Sandbox.open(client: @client, cpu: @cpu, mem_mb: @mem_mb, timeout_ms: @timeout, envs: @envs,
    ...
    @recipe.steps.each_with_index do |step, index|
      record, cwd = execute(sandbox, step, index, cwd, :step, results)
      results << record
    end

    # The verify phase runs only if setup actually completed. It answers a
    # different question from the steps: not "did the documented commands
    # succeed" but "does the thing they installed actually work". Without it
    # a green badge only ever meant a download finished.
    @verify.each_with_index do |command, offset|
      step = Recipe::Step.new(command: command, source_line: nil)
      record, cwd = execute(sandbox, step, @recipe.steps.size + offset, cwd, :verify, results)
      results << record
    end
  end
...
def execute(sandbox, step, index, cwd, phase, results)
  if halted?(results)
    return [Report::StepResult.new(step: step, index: index, status: :skipped, phase: phase,
                                   exit_code: nil, stdout: "", stderr: "", seconds: 0.0), cwd]
  end

  t0  = Time.now
  raw = sandbox.sh(wrap(step.command, cwd))
  seconds = (Time.now - t0).round(2)
  stdout, new_cwd = extract_cwd(raw.stdout, cwd)

  [Report::StepResult.new(step: step, index: index, phase: phase,
                          status: classify(raw.exit_code, stdout, raw.stderr),
                          exit_code: raw.exit_code, stdout: stdout, stderr: raw.stderr,
                          seconds: seconds), new_cwd]
end
